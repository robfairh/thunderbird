...
validParams<Equilibrium>()
{
  ...
  params.addRequiredCoupledVar("tau_xy", "shear stress in xy-direction");
  return params;
}
...
Equilibrium::Equilibrium(const InputParameters & parameters)
  : Kernel(parameters),
    ...
    _tau_xy(coupledValue("tau_xy")),
    _grad_tau_xy(coupledGradient("tau_xy"))
{
}
...
Equilibrium::computeQpResidual()
{
  ...
  if (_component == 0)
  {
    res += _grad_tau_xy[_qp](1) * _test[_i][_qp];
  }
  else if (_component == 1)
  {
    res += _grad_tau_xy[_qp](0) * _test[_i][_qp];
  }
