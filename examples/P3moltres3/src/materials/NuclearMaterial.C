...
registerMooseObject("ExampleApp", NuclearMaterial);
...
NuclearMaterial::NuclearMaterial(const InputParameters & parameters)
  : GenericConstantMaterial(parameters),
    ...
    _diffcoefA(declareProperty<std::vector<Real>>("diffcoefA")),
    _diffcoefB(declareProperty<std::vector<Real>>("diffcoefB")),
    ...
    _d_diffcoefA_d_temp(declareProperty<std::vector<Real>>("d_diffcoefA_d_temp")),
    _d_diffcoefB_d_temp(declareProperty<std::vector<Real>>("d_diffcoefB_d_temp")),
...
NuclearMaterial::dummyComputeQpProperties()
{
  ...
  for (decltype(_num_groups) i = 0; i < _num_groups; ++i)
  {
    ...
    _diffcoefA[_qp][i] = _xsec_linear_interpolators["DIFFCOEFA"][i].sample(dummy_temp);
    _diffcoefB[_qp][i] = _xsec_linear_interpolators["DIFFCOEFB"][i].sample(dummy_temp);
    ...
    _d_diffcoefA_d_temp[_qp][i] =
        _xsec_linear_interpolators["DIFFCOEFA"][i].sampleDerivative(dummy_temp);
    _d_diffcoefB_d_temp[_qp][i] =
        _xsec_linear_interpolators["DIFFCOEFB"][i].sampleDerivative(dummy_temp);
...
NuclearMaterial::splineComputeQpProperties()
{
  for (decltype(_num_groups) i = 0; i < _num_groups; ++i)
  {
    ...
    _fisse[_qp][i] = _xsec_spline_interpolators["FISSE"][i].sample(_temperature[_qp]) * 1e6 *
                     1.6e-19; // convert from MeV to Joules
    _diffcoefA[_qp][i] = _xsec_spline_interpolators["DIFFCOEFA"][i].sample(_temperature[_qp]);
    _diffcoefB[_qp][i] = _xsec_spline_interpolators["DIFFCOEFB"][i].sample(_temperature[_qp]);
    ...
    _d_fisse_d_temp[_qp][i] =
        ...
        1.6e-19; // convert from MeV to Joules
    _d_diffcoefA_d_temp[_qp][i] =
        _xsec_spline_interpolators["DIFFCOEFA"][i].sampleDerivative(_temperature[_qp]);
    _d_diffcoefB_d_temp[_qp][i] =
        _xsec_spline_interpolators["DIFFCOEFB"][i].sampleDerivative(_temperature[_qp]);
    _d_recipvel_d_temp[_qp][i] =
...
NuclearMaterial::monotoneCubicComputeQpProperties()
{
  for (decltype(_num_groups) i = 0; i < _num_groups; ++i)
  {
    ...
    _fisse[_qp][i] = _xsec_monotone_cubic_interpolators["FISSE"][i].sample(_temperature[_qp]) *
                     1e6 * 1.6e-19; // convert from MeV to Joules
    _diffcoefA[_qp][i] = _xsec_monotone_cubic_interpolators["DIFFCOEFA"][i].sample(_temperature[_qp]);
    _diffcoefB[_qp][i] = _xsec_monotone_cubic_interpolators["DIFFCOEFB"][i].sample(_temperature[_qp]);
    ...
    _d_fisse_d_temp[_qp][i] =
        ...
        1.6e-19; // convert from MeV to Joules
    _d_diffcoefA_d_temp[_qp][i] =
        _xsec_monotone_cubic_interpolators["DIFFCOEFA"][i].sampleDerivative(_temperature[_qp]);
    _d_diffcoefB_d_temp[_qp][i] =
        _xsec_monotone_cubic_interpolators["DIFFCOEFB"][i].sampleDerivative(_temperature[_qp]);
    _d_recipvel_d_temp[_qp][i] =
...
NuclearMaterial::linearComputeQpProperties()
{
  for (decltype(_num_groups) i = 0; i < _num_groups; ++i)
  {
    ...
    _fisse[_qp][i] = _xsec_linear_interpolators["FISSE"][i].sample(_temperature[_qp]) * 1e6 *
                     1.6e-19; // convert from MeV to Joules
    _diffcoefA[_qp][i] = _xsec_linear_interpolators["DIFFCOEFA"][i].sample(_temperature[_qp]);
    _diffcoefB[_qp][i] = _xsec_linear_interpolators["DIFFCOEFB"][i].sample(_temperature[_qp]);
    ...
    _d_fisse_d_temp[_qp][i] =
        ...
        1.6e-19; // convert from MeV to Joules
    _d_diffcoefA_d_temp[_qp][i] =
        _xsec_linear_interpolators["DIFFCOEFA"][i].sampleDerivative(_temperature[_qp]);
    _d_diffcoefB_d_temp[_qp][i] =
        _xsec_linear_interpolators["DIFFCOEFB"][i].sampleDerivative(_temperature[_qp]);
    _d_recipvel_d_temp[_qp][i] =
